...
func TestFreeSpinsGetSuccess(t *testing.T) {
	...
	code, content := SendRequest("GET",
		fmt.Sprintf("core/free_spins/get?session_token=%v", manager.GetSessionToken()), nil)
	if code != expectCode {
...
func TestFreeSpinsGetNoSession(t *testing.T) {
	...
	code, content := SendRequest("GET", "core/free_spins/get", nil)
	if code != expectCode {
		t.Errorf("received status %v, %v expected\n content: %v", code, expectCode, string(content))
	}
}
...
func TestFreeSpinsGetWrongSession(t *testing.T) {
	...
	code, content := SendRequest("GET", "core/free_spins/get?session_token=8d1a3d5c-17db-11ed-861d-0242ac120002", nil)
	if code != expectCode {
		t.Errorf("received status %v, %v expected\n content: %v", code, expectCode, string(content))
	}
}
