...
func getCellValues(r reflect.Value, tagName string) []string {
	...
	for j := 0; j < rt.NumField(); j++ {
		...
		if len(tags) > 1 && tags[1] != "" {
			nr, nf := getNestedStructField(r, field, tags[1])
			values = append(values, serializeValue(nr, nf.Name))

			continue
		}

		values = append(values, serializeValue(r, field.Name))
	}

	return values
}

func getNestedStructField(r reflect.Value, field reflect.StructField, nestedFieldName string) (reflect.Value, reflect.StructField) {
...
func serializeValue(r reflect.Value, fieldName string) string {
